...
public class AsteroidMoonGen {
	...
	public static class AsteroidGenSettings {
		...
		public NoiseConfig groundNoise      = new NoiseConfig(new OctaveNoise3D(16, 16, 16, 3, 2.0, 0.5)).setIsRidged(false).setNoiseStrength(0.35).setNoiseScale(0.85).setDistortStrength(0.5).setNoiseOffset(0.325);
		public NoiseConfig peakNoise        = new NoiseConfig(new OctaveNoise3D(16, 16, 16, 4, 2.0, 0.5)).setIsRidged(false).setNoiseStrength(0.125).setNoiseScale(0.64).setDistortStrength(0.5).setNoiseOffset(0.325);
		...
		public NoiseConfig colorNoise       = new NoiseConfig(new OctaveWorley(32, 32, 32, 5, 2.0, 0.5)).setIsRidged(false).setNoiseStrength(1.25).setNoiseScale(1.0).setDistortStrength(0.75).setNoiseOffset(0);
	...
	public static void main(String[] args) {
		try {
			...
			NoisemapGenerator.genNoisemap(shapeMap, settings.shapeNoise, null, resMul, true);
			for(int i = 0; i < width; i++) {
				for(int j = 0; j < height; j++) {
					shapeMap[i][j] = Math.min(1, Math.max(0, shapeMap[i][j]));
				}
			}
			MapUtils.displayMap("shape.png", shapeMap);
			
			System.out.println("Ground");
			NoisemapGenerator.genNoisemap(tempMap, settings.groundNoise, null, resMul, true);
			for(int i = 0; i < width; i++) {
				for(int j = 0; j < height; j++) {
					finalNoiseMap[i][j] = shapeMap[i][j] + tempMap[i][j];
				}
			}
			MapUtils.displayMap("ground.png", tempMap);
			
			System.out.println("Peaks");
			NoisemapGenerator.genNoisemap(tempMap, settings.peakNoise, shapeMap, resMul, true);
			for(int i = 0; i < width; i++) {
				for(int j = 0; j < height; j++) {
					finalNoiseMap[i][j] += tempMap[i][j];
				}
			}
			MapUtils.displayMap("peaks.png", tempMap);
			...
			MapUtils.save16Bit("asteroid_16.png", finalNoiseMap);
			...
			NoisemapGenerator.genNoisemap(tempMap2, settings.secondColorNoise, null, resMul, true);
			for(int i = 0; i < width; i++) {
				for(int j = 0; j < height; j++) {
